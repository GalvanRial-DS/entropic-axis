...
COLS = [
    ("Deep Anaesthesia", "Anestesia",     "Unconscious", "Awake", True,  "#E69F00"),
    ("Light Anaesthesia", "Anestesia",    "Sedation",    "Awake", True,  "#E69F00"),
    ("Anaes. Recovery",  "Anestesia",     "Recovery",    "Awake", True,  "#E69F00"),
    ("Modafinil",     "Modafinil",     "MOD",         "PLB",   False, "#F0E442"),
...
# Colorbar symmetric limits for each condition
DSW_LIMITS = {
    "Deep Anaesthesia":   (-0.054, 0.054),
    "Light Anaesthesia":  (-0.054, 0.054),
    "Anaes. Recovery":    (-0.054, 0.054),
...
def main():
    ...
    gs_top = gs[0].subgridspec(3, 7, wspace=0.3, hspace=0.08, height_ratios=[1, 1, 0.12])
    gs_bottom = gs[1].subgridspec(1, 7, wspace=0.35)
    
    axes_l = [fig.add_subplot(gs_top[0, i]) for i in range(7)]
    axes_z = [fig.add_subplot(gs_top[1, i]) for i in range(7)]
    axes_cb = [fig.add_subplot(gs_top[2, i]) for i in range(7)]
    
    axes_rc = [fig.add_subplot(gs_bottom[0, i]) for i in range(7)]
    
    for i, (label, csv_key, cond_a, cond_b, is_paired, color) in enumerate(COLS):
        # 1. Load data
        csv_path = os.path.join(csv_dir, "lono_all_data.csv")
        if not os.path.exists(csv_path):
            print(f"File not found: {csv_path}")
            continue
            
        df_all = pd.read_csv(csv_path, index_col=0).dropna(how="all").dropna(how="any")
        ds_key = csv_key if csv_key != "Schizophrenia" else "Schiz"
        df = df_all[df_all["dataset"] == ds_key].copy()
        ...
        df_plot = pd.DataFrame(plot_data, columns=["Network", "Delta"])
