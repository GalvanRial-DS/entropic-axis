...
DATASETS = [
    ...
    ("Anaesthesia (U)", "Anestesia", "Unconscious",  "Awake", True,  "#8e44ad"),
    ("Anaesthesia (S)", "Anestesia", "Sedation",     "Awake", True,  "#9b59b6"),
    ("Anaesthesia (R)", "Anestesia", "Recovery",     "Awake", True,  "#8e44ad"),
    ("Schizophrenia", "Schiz",      "SCHZ",         "CTRL",  False, "#2c3e50"),
    ("Modafinil",     "Modafinil",  "MOD",          "PLB",   False, "#27ae60"),
]

# Paths for dSW (called SE in filenames) and dFC
BASE_PATH = os.path.join(main_dir, "data/LONO_CSVs")
PATH_DSW = os.path.join(BASE_PATH, "lono_all_data.csv")
PATH_DFC = os.path.join(BASE_PATH, "dFC_lono_all_data.csv")
...
def main():
    ...
    results_dsw = []
    df_dsw = load_and_prep(PATH_DSW)
    
    for label, ds_key, cond_a, cond_b, paired, color in DATASETS:
        if df_dsw is not None:
            df = df_dsw[df_dsw.dataset == ds_key].copy()
    ...
    results_dfc = []
    df_dfc = load_and_prep(PATH_DFC)
    
    for label, ds_key, cond_a, cond_b, paired, color in DATASETS:
        if df_dfc is not None:
            df = df_dfc[df_dfc.dataset == ds_key].copy()
            # Check if conditions exist
            if cond_a not in df.condition.unique() or cond_b not in df.condition.unique():
                print(f"Skipping {label}: conditions {cond_a} or {cond_b} not found.")
                continue
